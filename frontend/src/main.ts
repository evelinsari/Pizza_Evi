...
function loadDetails() {
  ...
  let pizza = `${+number.value}  x  ${name.innerHTML}`
  let orderedPizza = getOrderedPizza(pizza)
  
  orderedPizzas.appendChild(orderedPizza)
}

function getOrderedPizza(pizza:string) {
  let orderedPizza = document.createElement("li")
  orderedPizza.setAttribute("class", "ordered")
  orderedPizza.innerHTML += pizza
  orderedPizza.appendChild(getRemoveButton())
...
orderButton.addEventListener("click", async function() {
  ...
  /*for (const property in order) {
    ...
    if (order[property] === null || (order[property]) === "") {
      let shouldSendOrder = true;
      shouldSendOrder = false;
...
function orderIsReadyPopUp() {
  let popup = document.getElementById("orderReady");
